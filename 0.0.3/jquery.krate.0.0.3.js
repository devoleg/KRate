(function( $ ) {
	...
	$.fn.krate = function( options ) {
		...
		if (
			...
			if (settings['size'] == undefined) settings['size'] = 16;
			if (settings['best'] == undefined) settings['best'] = 5;
			$(this).html('');
			$tmpelement = $('<div id="'+elementid+'"/>').appendTo(this);
			container = $tmpelement;
			container.css({'width':(settings['size']*settings['best'])+'px','background':'url('+settings['star-disabled']+') left center repeat-x','height':settings['size']+'px'})
			stars = new Array;
